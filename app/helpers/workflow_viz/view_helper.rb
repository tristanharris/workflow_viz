module WorkflowViz::ViewHelper
	...
	def render_super(current_path)
		...
		parent_view = view_paths.map do |path|
			path.find_all(File.basename(current_path).
								 split('.').first, lookup_context.prefixes.first, false, details).
									first.try(:identifier)
		end.compact.drop_while {|path| path != current_path}[1]
